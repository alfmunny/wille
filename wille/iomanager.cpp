...
#define LOG_EPOLL_CTL_ERROR(epfd, op, fd, event, rt) \
        WILLE_LOG_ERROR(g_logger) << "epoll_ctl(" << epfd << ", " \
            << (EpollCtlOp)op << ", " << fd << ", " << (EPOLL_EVENTS)event.events << "): " \
...
bool IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
    ...
    if(rt) {
        LOG_EPOLL_CTL_ERROR(m_epfd, op, fd, epevent, rt);
        return false;
    }
    ...
    if(cb) {
        ...
        WILLE_ASSERT2(event_ctx.fiber->getState() == Fiber::EXEC
    ...
    while(true) {
        ...
        for(int i = 0; i < rt; ++i) {
            ...
            if(event.events & (EPOLLERR | EPOLLHUP)) {
                WILLE_LOG_ERROR(g_logger) << "EPOLLERR or EPOLLHUP";
            ...
            if(rt2) {
                LOG_EPOLL_CTL_ERROR(m_epfd, op, fd_ctx->fd, event, rt2);
                continue;
            }
